...
#                                                     #
#                                                     #
#       Disco uses has_roles to determine whether it  #
#       has appropriate permissions to carry out      #
#       commands.                                     #
#                                                     #
#                                                     #
#######################################################
...
########################################################### Description for TSA.
#                                                         #
#       Disco's method to check whether or not a user     #
#       is considered a long-time member or not. A long   #
#       -time member will be less-harshly punished for    #
#       chat infractions.                                 #
#                                                         #
###########################################################
...
#       !!MUTE FUNCTION:                                     #
#                                                            #
#       Parameters Set 1: +, -, add, remove                  #
#                                                            #
#       Parameters Set 2: @Username                          #
#                                                            #
#       Parameters Set 3: <time in seconds>                  #
#                                                            #
#       Returns: Assigns @Username "Muted" role.             #
#                                                            #
#       Exceptions: Returns unable to assign Muted.          #
#                                                            #
##############################################################
    async def handle_mute(self, message, server, author, mentions, option, time=None, reason=None):
        ...
        if self.has_roles(message.channel, author, server):
            ...
            if not mentions: #If you did not mention any user in your mute command...[TSA]
                    raise CommandError('Invalid user specified')
            if option in ['+', 'add']: #If you want to add someone to the mute list...[TSA]
                ...
                if time:
                    try:
                        ...
                        raise CommandError('Time provided invalid:\n{}\n'.format(time)) #Time must be a digit number. If invalid... [TSA]
                await self.write_to_modlog(message, author, server, reason) #Write this command to the modlog when carried out. [TSA]
                mutedrole = discord.utils.get(server.roles, name='Muted') #Assign role "Muted" to user muted. [TSA]
                if not mutedrole:
                    raise CommandError('No Muted role created') #If no "Muted" role exists...[TSA]
                ...
                if time:
                    ...
                    for user in mentions:
                        ...
                        await self.server_voice_state(user, mute=False) #Releases "Muted" role after assigned time in seconds. [TSA]
            else:
                if time:
...
#       Parameters Set 2: @Username                                              #
#                                                                                #
#       Parameters Set 3: "<role name>"                                          #
#                                                                                #
#       Returns: Assigns specified user <role name>.                             #
#                                                                                #
#       Notes: <role name> is case sensitive, and requires quotation marks       #
#       and spaces if the role title is multiple words.                          #
#                                                                                #                                                                             #
#       Exceptions: Returns unable to add or remove role to @Username.           #
#                                                                                #
##################################################################################
...
#       !!PURGE FUNCTION:                                                              #
#                                                                                      #
#       Parameters Set 1: <number of messages to remove>                               #
#                                                                                      #
#       Parameters Set 2: @Username                                                    #
#           -> @Username parameter is optional.                                        #
#                                                                                      #
#       Returns: Removes given number of messages either in that specific chat         #
#       or to that specific user.                                                      #
#                                                                                      #
#       Exceptions: Returns invalid user specified or invalid message count.           #
#                                                                                      #
########################################################################################
...
#       Parameters Set 2: @Username                                    #
#                                                                      #
#       Parameters Set 3: "<reason>"                                   #
#                                                                      #
#       Returns: Bans specified user, preventing them from             #
#       rejoining the server with an invite code. This ban is          #
#       IP-based.                                                      #
#                                                                      #
#       Exceptions: Returns invalid user specified.                    #
#                                                                      #
########################################################################
...
#       Parameters Set 3: "<reason>"                                   #
#                                                                      #
#       Returns: Kicks specified user from the server. They can        #
#       only rejoin with an invite link.                               #
#                                                                      #
#       Exceptions: Returns invalid user specified.                    #
#                                                                      #
########################################################################
...
#       Parameters Set 1: +, -, add, remove                            #
#                                                                      #
#       Parameters Set 2: @Username, "<role name>"                     #
#                                                                      #
#       Parameters Set 3: "<reason>"                                   #
#                                                                      #
#       Returns: Adds specified user or specified role to bot's        #
#       chat filter whitelist. Being on the whitelist prevents the     #
#       bot from punishing users that type blacklisted words. This     #
#       can be applied to one single user or an entire role group      #
#                                                                      #
#       Exceptions: Returns invalid user/role specified.               #
#                                                                      #
########################################################################
...
#       Parameters Set 3: "<reason>"                                         #
#                                                                            #
#       Returns: Gives specified user or role permissions to Disco's         #
#       commands.                                                            #
#                                                                            #
#       Exceptions: Returns invalid user/role specified.                     #
#                                                                            #
...
#       Parameters Set 1: +, -, add, remove                                #
#                                                                          #
#       Parameters Set 2: "<word or phrase>"                               #
#                                                                          #
#       Parameters Set 3: "<reason>"                                       #
#                                                                          #
#       Returns: Adds specified string to Disco's blacklist. A user        #
#       that submits that string to the chat will be punished              #
#       according to Disco's registration settings.                        #
#                                                                          #
#       Exceptions: Returns nonexistent blacklisted string.                #
#                                                                          #
############################################################################
...
#       !!SETTOKENS FUNCTION                                               #
#                                                                          #
#       Parameters Set 1: <number digit>                                   #                                                                                               
#                                                                          #
#       Parameters Set 2: "<reason>"                                       #
#                                                                          #
#       Returns: Sets specified number of tokens (messages) a user can     #
#       spend (send) in a given time.                                      #
#                                                                          #
#       Exceptions: Returns invalid number, or < 1 error.                  #
#                                                                          #
############################################################################
...
############################################################################ Description for TSA.
#                                                                          #
#       !!SETTOKENRESET FUNCTION                                           #
#                                                                          #
#       Parameters Set 1: <time in seconds>                                #                                                                                               
#                                                                          #
#       Parameters Set 2: "<reason>"                                       #
#                                                                          #
#       Returns: Sets a number of seconds a user can send a given number   #
#       of messages before they're limited from sending more messages      #
#       until the token reset time is over.                                #
#                                                                          #
#       Exceptions: Returns invalid number, or < 1 error.                  #
#                                                                          #
############################################################################
...
#                                                                          #
#       Parameters Set 1: kick, ban, mute, nothing                         #                                                                                               #
#                                                                          #
#       Parameters Set 2: "<reason>"                                       #
#                                                                          #
#       Returns: Sets what action the bot will carry out when a user       #
#       submits a blacklisted word to the chat.                            #
#                                                                          #
#       Exceptions: Returns invalid command issued.                        #
#                                                                          #
############################################################################
...
############################################################################ Description for TSA.
#                                                                          #
#       !!SETLONGTIMEMEMBER FUNCTION                                       #
#                                                                          #
#       Parameters Set 1: <time in hours>                                  #                                                                                               #
#                                                                          #
#       Parameters Set 2: "<reason>"                                       #
#                                                                          #
#       Returns: Sets the number of hours after which a user will receive  #
#       less-harsh filtering on chat infractions (duplicate messages,      #
#       duplicate characters).                                             #
#                                                                          #
#       Exceptions: Returns number < 0, or invalid number issued.          #
#                                                                          #
############################################################################
...
#       !!SETMODLOG FUNCTION                                               #
#                                                                          #
#       Parameters Set 1: <channel ID>                                     #                                                                                               #
#                                                                          #
#       Parameters Set 2: "<reason>"                                       #
#                                                                          #
#       Returns: Sets the channel in which the bot will report all         #
#       moderator functions, mutes, bans, and kicks.                       #
#                                                                          #
#       Exceptions: Returns invalid channel ID specified.                  #
#                                                                          #
############################################################################
...
#                                                                          #
#       Returns: Sets the channel in which the bot will report all         #
#       changes to the chat (edited messages, changed profile pictures,    #
#       changed usernames, etc.).                                          #
#                                                                          #
#       Exceptions: Returns number < 0, or invalid number issued.          #
#                                                                          #
############################################################################
...
############################################################################ Description for TSA.
#                                                                          #
#       !!TOGGLEMODLOG FUNCTION                                            #
#                                                                          #
#       Returns: Activates or deactivates use of the modlog.               #
#                                                                          #
############################################################################
...
############################################################################ Description for TSA.
#                                                                          #
#       !!TOGGLESERVERLOG FUNCTION                                         #
#                                                                          #
#       Returns: Activates or deactivates use of the serverlog.            #
#                                                                          #
############################################################################
...
############################################################################ Description for TSA.
#                                                                          #
#       !!TOGGLEANTITWITCH FUNCTION                                        #
#                                                                          #
#       Returns: Activates or deactivates removal of Twitch.tv emote       #
#       phrases in chat.                                                   #
#                                                                          #
############################################################################
...
#       !!SERVERINFO FUNCTION                                              #
#                                                                          #
#       Returns: Sends issuer of command a private message with all        #
...
#       !!ALERTRHINO FUNCTION                                              #
#                                                                          #
#       Returns: Prints an alert message in console and in bot creator's   #
#       Discord server if there are major issues with the bot              #
#                                                                          #
############################################################################
...
#       !!PING FUNCTION                                                    #
#                                                                          #
#       Returns: "PONG!" to determine bot's responsiveness.                #
#                                                                          #
############################################################################
...
#       !!HELP FUNCTION                                                    #
#                                                                          #
#       Returns: A link to the bot creator's github repository.            #
#                                                                          #
############################################################################
...
#       !!INFO FUNCTION                                                    #
#                                                                          #
#       Returns: Prints relevant information about bot in chat.            #
#                                                                          #
############################################################################
...
#       !!DONATE FUNCTION                                                  #
#                                                                          #
#       Returns: Prints bot creator's Patreon into chat.                   #
#                                                                          #
############################################################################
...
#       Parameters Set 1: +, -, add, remove                                #
#                                                                          #
#       Parameters Set 2: <channel ID>                                     #
#                                                                          #
#       Parameters Set 3: "<reason>"                                       #
#                                                                          #
#       Returns: Ignores or unignores specified chat channel from the      #
#       bot's chat filter.                                                 #
#                                                                          #
#       Exceptions: Invalid channel specified.                             #
#                                                                          #
############################################################################
...
#       !!ROLECOLOR FUNCTION                                               #
#                                                                          #
#       Parameters Set 1: "<role name>"                                    #
#                                                                          #
#       Parameters Set 2: "<# hex color code>"                             #
#                                                                          #
#       Parameters Set 3: "<reason>"                                       #
#                                                                          #
#       Returns: Sets specified role name specified color.                 #
#                                                                          #
#       Exceptions: Invalid role, colorcode specified.                     #
#                                                                          #
############################################################################
    async def handle_rolecolor(self, message, author, server, rolename, new_hex, reason=None):
        """
        Usage: {command_prefix}rolecolor "<role name>" "<# hex code>" "<reason>"
...
#       !!BROADCAST FUNCTION                                               #
#                                                                          #
#       Parameters Set 1: "<message>"                                      #
#                                                                          #
#       Returns: Prints a message to all servers bot exists in.            #
#                                                                          #
#       Exceptions: Prints message unable to send message to server.       #
#                                                                          #
############################################################################
...
#       !!CLEANSE FUNCTION                                                 #
#                                                                          #
#       Returns: Removes bot from servers registration is not complete on. #
#                                                                          #
############################################################################
    async def handle_cleanse(self, message, author, server):
        """
        Usage: {command_prefix}cleanse
        ...
        if author.id == self.config.master_id:
            ...
            if server_leave_array:
                for dbserver in server_leave_array:
                    print('Leaving Dead Server : {}'.format(dbserver.name))
...
#       !!ID FUNCTION                                                      #
#                                                                          #
#       Returns: Prints user's unique ID.                                  #
#                                                                          #
############################################################################
...
#                                                                          #
#       Returns: Forces backup of all server configurations.               #
#                                                                          #
############################################################################
...
#       !!CHANGEGAME FUNCTION                                              #
#                                                                          #
#       Returns: Sets new currently playing game for the bot.              #
#                                                                          #
############################################################################
...
#       !!EVAL FUNCTION                                                    #
#                                                                          #
#       Parameters Set 1: "<string>"                                       #
#                                                                          #
#       Returns: Runs "string" through eval parameters for testing.        #
#                                                                          #
############################################################################
...
#       !!LURK FUNCTION                                                    #
#                                                                          #
#       Returns: Forces bot to send 0 messages to a server, and not time   #
#       out.                                                               #
#                                                                          #
############################################################################
    async def handle_lurk(self, author, server, message, channel):
        ...
        Force the bot to lurk in a server rather than send messages to it or leave
...
#       !!REMIND FUNCTION                                                  #
#                                                                          #
#       Returns: Reminds all server owners to complete bot registration.   #
#                                                                          #
############################################################################
...
#       !!JOINSERVER FUNCTION                                              #
#                                                                          #
#       Parameters Set 1: "<server invite link>"                           #
#                                                                          #
#       Returns: Forces bot to join specified server.                      #
#                                                                          #
############################################################################
    ...
    async def on_message(self, message, flag=None):
        ...
        if message.channel.is_private:
            if message.author.id == '105309315895693312' or message.author.id == '104867073343127552': 
                return
            try:
